...
## 当前状态：⑧ 检索 v2（in-batch 负例）⏳ 脚本就绪待跑；先 scp 消融结果
...
| ⑧ 检索 v2（in-batch 负例） | ⏳ 脚本已就绪（`nc_retrieve_*_patent_v2.sh`） | `ckpt/patent/nc_retrieval_v2/` | max_len 128 + batch 4 恢复 in-batch 负例（②' 只有 5 路对比）；训练 ~8h → infer ~0.5h → search ~2h → 重测 recall，**大涨才**级联重训重排 |
